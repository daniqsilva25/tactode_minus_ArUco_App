...
const newTile = (rect = new cv.Rect()) => {
  return {
    rect: rect,
    tactode: {
      type: '',
...
function run (img = '', debug = false, td = new params.HOGSVM4detection(), tc = new params.HOGSVM()) {
  ...
  while (rotationStep < 4 && isRotated) {
    ...
    // -> Get bigger and smaller normal sized tiles
    for (let i = 0; i < cntArr.length; ++i) {
      const c = cntArr[i]
  ...
  // -> Put tiles in the correct positions
  cntArr.sort((c0, c1) => c0.rect.y - c1.rect.y)
  const rectsArr = []; let line = []
  let prevCnt
  cntArr.forEach(currCnt => {
    if (prevCnt === undefined) {
      ...
      if (currCnt.rect.y > prevCnt.rect.y + prevCnt.rect.height / 4) { // new line found
        ...
        line = line.map(c => newTile(c.rect))
        rectsArr.push(line)
        line = []
      } else {
        line.push(prevCnt)
