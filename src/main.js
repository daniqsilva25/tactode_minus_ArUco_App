...
// FUNCTIONS
function showContours (cnts = [], mat = new cv.Mat()) {
...
function drawRects (arr = [], mat = new cv.Mat()) {
  arr.forEach(elem => {
    if (elem.length >= 1) {
...
// -> Rotating picture (-90 * i)º if needed
let rotationStep = 0
let isRotated = true
...
svmDetector.load(params.hogsvmDetection.svm)
const hogDetector = new cv.HOGDescriptor()
hogDetector.load(params.hogsvmDetection.hog)
while (rotationStep < 4 && isRotated) {
  ...
  // -> Get bigger and smaller normal sized pieces
  for (let i = 0; i < cntArr.length; ++i) {
    const c = cntArr[i]
  ...
  for (let i = cntArr.indexOf(bigger); i <= cntArr.indexOf(smaller); ++i) {
    ...
    const roi = src.getRegion(new cv.Rect(x, y, width, height))
      .resize(
        params.hogsvmDetection.size.height,
        params.hogsvmDetection.size.width,
        0, 0, cv.INTER_AREA
      )
    const descriptors = hogDetector.compute(roi)
    const prediction = svmDetector.predict(descriptors)
...
if (debug) {
  cv.imwrite(path.join(debugFolder, 'img_bw.jpg'), fg)
}

// -> Remove inner and noisy contours
for (let i = 0; i < cntArr.length; ++i) {
...
if (debug) {
  const mat = showContours(contours.filter(c => c.numPoints !== 0), src.copy(src))
  cv.imwrite(path.join(debugFolder, 'img_cnts.jpg'), mat)
}
cntArr = cntArr.filter(c => c.idx !== -1)

// -> Agglomerate little bits of remaining contours
...
// -> Put pieces in the correct positions
...
cntArr.forEach(currCnt => {
  if (prevCnt === undefined) {
    ...
    if (currCnt.rect.y > prevCnt.rect.y + prevCnt.rect.height / 4) { // new line found
      line.push(prevCnt)
      line.sort((c0, c1) => c0.rect.x - c1.rect.x)
      line = line.map(c => newPiece(c.rect))
      pieces.push(line)
      line = []
    } else {
      line.push(prevCnt)
...
let invalidRoiFound = false
pieces.forEach(line => {
  line.forEach(piece => {
    const l = 0
    piece.rect = toSquare(new cv.Rect(piece.rect.x - l, piece.rect.y - l,
      piece.rect.width + 2 * l, piece.rect.height + 2 * l))
    if (piece.rect.x < 0 || piece.rect.y < 0 || piece.rect.x + piece.rect.width > src.cols || piece.rect.y + piece.rect.height > src.rows) {
      invalidRoiFound = true
    }
  })
})
if (debug) {
  const mat = drawRects(pieces, src.copy(src))
...
if (!invalidRoiFound) {
  ...
  svmClassifier.load(params.hogsvmClassification.svm)
  const hogClassifier = new cv.HOGDescriptor()
  hogClassifier.load(params.hogsvmClassification.hog)
  pieces.forEach(line => {
    line.forEach(tile => {
      const roi = src.getRegion(tile.rect)
      const resized = roi.resize(
        params.hogsvmClassification.size,
        params.hogsvmClassification.size,
        0, 0, cv.INTER_AREA
      )
      const descriptors = hogClassifier.compute(resized)
      const prediction = svmClassifier.predict(descriptors)
      const typeIndex = Math.floor(prediction / 100)
      const pieceIndex = prediction % 100
      const classLine = fs.readFileSync(
        params.hogsvmClassification.classes
      ).toString().split('\n')[typeIndex].split(':')
...
// -> Print results
const numPieces = cntArr.length
let resultStr = ` Number of tiles found: ${numPieces}\n\n `
pieces.forEach(line => {
  line.forEach(piece => {
...
console.log(`finished: ${params.getCurrentTime()}\n+++`)
